...
export default function Home() {
  ...
  const sections = [
    { title: "Trending Now", stories: MOCK_STORIES.slice(1, 5) },
    { title: "Lemonade Originals", stories: MOCK_STORIES.filter(s => s.isOriginal) },
    { title: "African Fantasy", stories: MOCK_STORIES.filter(s => s.genre === "African Fantasy") },
    { title: "Sci-Fi & Cyberpunk", stories: MOCK_STORIES.filter(s => s.genre === "Sci-Fi & Cyberpunk") },
  ];
    ...
    <div className="flex flex-col w-full min-h-full pb-20">
      ...
      <div className="flex flex-col gap-12 mt-8">
        ...
        <section className="px-6 md:px-12">
          ...
          <Link to={`/story/s2`} className="group flex items-center gap-4 bg-ink-deep p-4 rounded-2xl border border-white/5 hover:border-lemon-muted/30 transition-colors w-full max-w-xl">
             <div className="w-16 h-20 rounded-xl overflow-hidden shrink-0 bg-black">
               <img src={MOCK_STORIES[1].coverImage} alt="Story" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" referrerPolicy="no-referrer" />
             </div>
             <div className="flex-1 flex flex-col justify-center min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-semibold text-lemon-muted uppercase tracking-wider">Chapter 14</span>
                  <span className="text-xs text-white/50">80%</span>
                </div>
                <h3 className="font-display font-bold text-lg truncate">{MOCK_STORIES[1].title}</h3>
                <div className="w-full h-1 bg-black rounded-full overflow-hidden mt-3">
                  <div className="h-full bg-lemon-muted w-[80%]" />
                </div>
             </div>
          </Link>
        </section>
