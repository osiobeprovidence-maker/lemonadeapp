...
export default function AdminFeatured() {
  ...
  useEffect(() => {
    setFeaturedStories(stories.filter((story) => story.isFeatured));
  }, [stories]);
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-white/40 font-bold uppercase tracking-widest text-xs mt-1">
            Curate the platform homepage experience
          </p>
        </div>
        ...
        <div className="xl:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-white/30">
              {featuredStories.length} Curated Items
            </span>
          </div>
              ...
              >
                <div className="flex flex-col gap-1 text-white/20">
                  <button
                    ...
                    className="hover:text-lemon-muted transition-colors"
                  >
                    <ArrowUp size={16} />
                  </button>
                  <button
                ...
                <div className="hidden md:flex flex-col items-end gap-2 px-6">
                  <span className="text-[9px] font-black uppercase tracking-widest px-2 py-1 bg-white/5 text-white/40 rounded border border-white/5">
                    {story.genre}
                  </span>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            {[
              ...
              {
                label: "Originals",
                active: "Studio curated",
                icon: BadgeCheck,
              },
              { label: "New Drops", active: "Recent published", icon: Box },
              {
                label: "Featured Creators",
                active: "Manual choice",
                icon: Layout,
              },
            ].map((module, i) => (
              <div
                key={i}
                className="p-6 bg-ink-deep border border-white/5 rounded-[2rem] hover:border-white/20 transition-all group"
              >
                <div className="flex items-center justify-between">
                  ...
                  <button className="text-[10px] font-black uppercase tracking-widest text-lemon-muted hover:underline">
                    Edit
                  </button>
                </div>
              </div>
          ...
          <div className="p-8 bg-ink-deep border border-white/5 rounded-[3rem] text-center">
            <p className="text-xs font-bold text-white/30 italic">
              Changes to featured content reflect instantly on the user
